...
const CONTACT_ITEMS = [
  ...
  { icon: "🌐", label: "web", value: "dcoder.online", href: "dcoder.online" },
] as const;
  ...
  return (
    <div className="border-b border-dashed border-black/[0.14]">
      <button
        ...
        aria-expanded={open}
      >
        <span className="font-mono text-[11px] text-dc-ink2 group-hover:text-dc-ink transition-colors leading-relaxed">
          {q}
        </span>
        <motion.div
          animate={{ rotate: open ? 45 : 0 }}
          transition={{ duration: 0.22 }}
          className="w-5 h-5 shrink-0 bg-dc-ink text-dc-cream flex items-center justify-center text-base font-light leading-none mt-0.5"
        >
          +
        </motion.div>
      </button>
      <AnimatePresence initial={false}>
        {open && (
          <motion.p
            ...
            className="font-mono text-[10px] text-dc-ink3 leading-[1.9] overflow-hidden pb-3 pr-6"
          >
            {a}
          </motion.p>
        )}
...
const inputCls = `
  font-mono text-[12px] text-dc-ink
...
const Field = ({
  ...
  <div className="flex flex-col gap-1.5">
    <label
      htmlFor={id}
      className="font-mono text-[10px] tracking-[0.12em] uppercase text-dc-ink3"
    >
      {label} *
    </label>
    {children}
        ...
        <div className="max-w-[1300px] mx-auto px-5 lg:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14 items-start">
            ...
            <div className="flex flex-col gap-7">
              ...
              >
                <p className="font-mono text-[10px] tracking-[0.2em] uppercase text-dc-ink3 mb-3">
                  {"// reach us directly"}
                </p>
                <ul className="flex flex-col">
                  {CONTACT_ITEMS.map(({ icon, label, value, href }) => (
                    <li
                      key={value}
                      className="flex items-center gap-3 py-3 border-b border-dashed border-black/[0.12] first:border-t group"
                    >
                      <span
                        className="text-[15px] shrink-0 w-6 text-center"
                        aria-hidden
                      >
                        {icon}
                      </span>
                      ...
                      <div className="flex flex-col gap-0.5">
                        <span className="font-mono text-[9px] tracking-widest uppercase text-dc-ink3/50">
                          {label}
                        </span>
                        {href ? (
                          <a
                            ...
                            className="font-mono text-[12px] text-dc-ink2 hover:text-dc-red transition-colors duration-200"
                          >
                            {value}
                          </a>
                        ) : (
                          <span className="font-mono text-[12px] text-dc-ink2">
                            {value}
                          </span>
                        )}
                      </div>
              ...
              {/* Sticky note */}
              <div
                className="inline-block relative bg-dc-sticky-yellow border border-black/[0.07] p-4 -rotate-[1.5deg] max-w-[220px]"
                style={{ boxShadow: "3px 4px 0 rgba(0,0,0,0.08)" }}
              >
                <Tape rotate="rotate-[3deg]" width={46} />
                <p className="font-hand text-[16px] text-dc-ink2 leading-[1.5]">
                  avg response under 24 hrs ⚡
                </p>
                <p className="font-hand text-[13px] text-dc-ink3 mt-1">
            ...
            >
              ...
              >
                <Tape rotate="-rotate-[2.5deg]" width={68} />
                ...
                {sent ? (
                  ...
                  >
                    ...
                    <h3 className="font-serif font-bold text-dc-ink text-xl">
                      message received!
                    </h3>
                    <p className="font-mono text-[11px] text-dc-ink3 max-w-sm leading-relaxed">
                      we&apos;ve got your enquiry and will get back within 24
                      hours. exciting things ahead!
                    </p>
                    <button
                      onClick={() => setSent(false)}
                      className="font-mono text-[11px] uppercase tracking-wider border-2 border-dc-ink px-5 py-2.5 hover:bg-dc-ink hover:text-dc-cream transition-all duration-200 mt-1"
                    >
                      send another →
                    </button>
                  </motion.div>
                  ...
                  <div className="p-6 lg:p-8">
                    {/* Form header */}
                    <div className="flex items-center justify-between pb-4 mb-5 border-b border-dashed border-black/[0.1]">
                      <div>
                        <p className="font-hand text-[19px] text-dc-ink font-bold">
                          get a free quote ✦
                        </p>

                        <p className="font-mono text-[10px] text-dc-ink3 mt-0.5">
                    ...
                    >
                      ...
                      <Field label="Service you need" id="expertise">
                        <div className="relative">
                          ...
                          <span className="absolute right-3 top-1/2 -translate-y-1/2 text-dc-ink3 pointer-events-none text-[11px]">
                            ▾
                          </span>
                        </div>
                      </Field>
                      ...
                      <Field label="Tell us about your project" id="project">
                        ...
                        />
                        <p className="font-mono text-[9px] text-dc-ink3/40 text-right mt-0.5">
                      ...
                      <button
                        type="submit"
                        disabled={sending}
                        className="group relative font-mono text-[12px] font-bold uppercase tracking-[0.15em] bg-dc-ink text-dc-cream w-full py-3 overflow-hidden disabled:opacity-40 disabled:cursor-not-allowed mt-0.5"
                      >
              ...
              <p className="font-mono text-[10px] text-dc-ink3/40 text-center mt-3 tracking-wide">
